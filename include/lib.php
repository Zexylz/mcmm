...
function saveModMetadata($serverId, $modId, $platform, $modName, $fileName, $fileId = null, $logo = null, $extraData = [])
{
    ...
    // Use flock to safely update metadata
    $fp = fopen($metaFile, 'c+');
    if ($fp && flock($fp, LOCK_EX)) {
        ...
        $modsData = json_decode($currentJson, true) ?: [];
        ...
        ftruncate($fp, 0);
        rewind($fp);
        if (fwrite($fp, json_encode($modsData, JSON_PRETTY_PRINT)) === false) {
            dbg("ERROR: Failed to write metadata to $metaFile");
        }
        fflush($fp);
        flock($fp, LOCK_UN);
    } else {
        dbg("ERROR: Failed to acquire lock for $metaFile");
    }
    if ($fp)
        fclose($fp);
